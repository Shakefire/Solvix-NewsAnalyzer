...
st.markdown("Advanced AI-powered system to detect fake news and analyze article credibility")
...
if page == "Main Classifier":
        ...
        if st.button("🔍 Analyze Article", type="primary", disabled=not news_text or len(news_text.split()) < 50):
            if news_text:
                with st.spinner("Analyzing article..."):
                    # Get prediction
                    prediction, confidence, feature_importance = model.predict(news_text)
                    ...
                    with result_col1:
                        ...
                        if prediction == 'Real':
                            st.success(f"**Classification: {prediction} News**")
                            color = "green"
                        else:
                            ...
                            with exp_col1:
                                ...
                                with exp_col2:
                                    ...
                                    for word, importance in sorted_features:
                                        ...
                                        st.write(f"**{word}**: {abs(importance):.3f} {direction}")
                    ...
                    with trust_col1:
                        ...
                        if trust_score >= 0.7:
                            st.success(f"**Trust Score: {trust_score:.1%}** - High Credibility")
                        elif trust_score >= 0.4:
                            st.warning(f"**Trust Score: {trust_score:.1%}** - Medium Credibility")
                        else:
                            st.error(f"**Trust Score: {trust_score:.1%}** - Low Credibility")
    ...
    with col2:
        ...
        if total_predictions > 0:
            ...
            real_count = sum(1 for p in recent_predictions if p['prediction'] == 'Real')
    ...
    if st.session_state.prediction_history:
        ...
        if filter_feedback != "All":
            filtered_df = filtered_df[filtered_df['User Feedback'] == filter_feedback]
        ...
        if not filtered_df.empty:
            ...
            with stat_col4:
                helpful_feedback = (filtered_df['User Feedback'] == '👍 Helpful').sum()
                            ...
                            if st.button(f"🔍 Fact-Check Claim {i+1}", key=f"factcheck_{i}"):
                                with st.spinner("Checking facts..."):
                                    ...
                                    if fact_check_result['status'] == 'Verified':
                                        st.success(f"✅ **{fact_check_result['status']}**")
                                    elif fact_check_result['status'] == 'False':
                                        st.error(f"❌ **{fact_check_result['status']}**")
                                    elif fact_check_result['status'] == 'Partially True':
                                        st.warning(f"⚠️ **{fact_check_result['status']}**")
                                    else:
                                        st.info(f"❓ **{fact_check_result['status']}**")
                                    
                                    st.write(f"**Explanation:** {fact_check_result['explanation']}")
                                    st.write(f"**Sources:** {', '.join(fact_check_result['sources'])}")
    else:
        st.info("No articles analyzed yet. Go to the Main Classifier to analyze some articles first!")

elif page == "Geolocation Analysis":
    st.header("🗺️ Geolocation Claim Map")
    
    st.write("This feature visualizes claimed event locations mentioned in news articles.")
    ...
    if st.session_state.prediction_history:
        ...
        sample_locations = [
            {"lat": 40.7128, "lon": -74.0060, "name": "New York", "articles": 3, "status": "Verified"},
            {"lat": 51.5074, "lon": -0.1278, "name": "London", "articles": 2, "status": "Disputed"},
            {"lat": 35.6762, "lon": 139.6503, "name": "Tokyo", "articles": 1, "status": "Unverified"},
            {"lat": 48.8566, "lon": 2.3522, "name": "Paris", "articles": 2, "status": "Verified"},
        ]
        ...
        if map_data['last_object_clicked_popup']:
            ...
            st.info("Click on a marker to see details about claims from that location.")
        ...
        with stat_col2:
            verified_count = sum(1 for loc in sample_locations if loc["status"] == "Verified")
            st.metric("Verified", verified_count)
        with stat_col3:
            disputed_count = sum(1 for loc in sample_locations if loc["status"] == "Disputed")
            st.metric("Disputed", disputed_count)
        with stat_col4:
            unverified_count = sum(1 for loc in sample_locations if loc["status"] == "Unverified")
            st.metric("Unverified", unverified_count)
    
    else:
        st.info("No articles analyzed yet. Analyze some articles to see geolocation data!")

elif page == "Chat Verification":
    st.header("💬 Interactive Chatbot for Verification")
    
    st.write("Ask specific questions about article content and get real-time fact-check information.")
...
def generate_fact_check_response(prompt):
    ...
    political_keywords = ['election', 'vote', 'politician', 'government', 'policy']
...
with st.sidebar:
    ...
    if total_analyzed > 0:
        real_news_count = sum(1 for p in st.session_state.prediction_history if p['prediction'] == 'Real')
        ...
        if len(st.session_state.user_feedback) > 0:
            user_satisfaction = helpful_feedback / len(st.session_state.user_feedback) * 100
